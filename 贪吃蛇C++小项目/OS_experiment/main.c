...
int main()  // 替换void main为标准int main
{
    int i, j, choice1;
    ...
    // 创建目录（避免文件打开失败）
    system("md c:\\osfile >nul 2>&1");
    system("md c:\\osfile\\file >nul 2>&1");
    ...
    if (loginsuc == 1)  /*Login Successfully*/
    {
        while (1)
        {
            ...
            if (choiceend == 1)
            {
                printf("\n\nC:\\%s>", my_strupr(dirname));
            }
            else printf("Bad command or file name.\nC:\\%s>", my_strupr(username));

            // 替换gets为fgets（解决安全警告）
            fgets(choice, sizeof(choice), stdin);
            choice[strcspn(choice, "\n")] = '\0'; // 去除换行符
            strcpy(choice, ltrim(rtrim(my_strlwr(choice)))); // 替换strlwr为my_strlwr
            ...
            case 9:printf("\nYou have exited this system.");
                QuitF(); exit(0); break;
            case 10:choiceend = 1; system("cls"); break; // 替换clrscr为system("cls")
...
void LoginF()  /*LOGIN FileSystem*/
{
    ...
    while (1)
    {
        findout = 0;
        printf("\n\nLogin Name:");
        fgets(loginame, sizeof(loginame), stdin); // 替换gets
        ...
        for (i = 0; fread(&ufd_lp, sizeof(OSUFD_LOGIN), 1, fp_mfd) != 0; i++)
            if (strcmp(my_strupr(ufd_lp.ufdname), my_strupr(loginame)) == 0)
            {
                findout = 1;
        ...
        if (findout == 1)  /*user exist*/
        {
            ...
            {
                strcpy(username, my_strupr(loginame));
                ...
                for (j = 0; fread(&ufd_lp, sizeof(OSUFD_LOGIN), 1, fp_mfd) != 0; j++)
                {
                    ...
                    strcpy(ufd[j]->ufdname, my_strupr(ufd_lp.ufdname));
                    fp_ufd = fopen(str, "rb");
                    fcount[j] = 0;
            ...
            else
            {
                ...
                while (flag)
                {
                    ...
                    fgets(a, sizeof(a), stdin); // 替换gets
                    a[strcspn(a, "\n")] = '\0';
                    ltrim(rtrim(a));
                    if (strcmp(my_strupr(a), "Y") == 0) { loginsuc = 0; flag = 0; }
                    else if (strcmp(my_strupr(a), "N") == 0) { loginsuc = 0; flag = 0; return; }
                }
            }
        }
        else
        {
            ...
            if (strcmp(loginpw, logincpw) == 0)
            {
                strcpy(ufd_lp.ufdname, my_strupr(loginame));
...
void InputPW(char* password)  /*input password,use '*' replace*/
{
    ...
    for (j = 0; j <= 7; j++)
    {
        password[j] = _getch(); // 替换getch为_getch
...
void DirF()  /*Dir FileSystem*/
{
    ...
    if (strcmp(my_strupr(ltrim(rtrim(dirname))), "") != 0)
    {
        printf("\n\nC:\\%s>dir\n", dirname);
        ...
        for (i = 0; i < fcount[j]; i++)
        {
            if ((i % 16 == 0) && (i != 0))
            {
                ...
                _getch(); // 替换getch为_getch
                printf("\n%14s%16s%14s%10s%18s\n", "FileName", "FileAddress", "FileLength", "Type", "FileMode");
            }
            _itoa(ufd[j]->ufdfile[i].fpaddr, str, 10); // 替换itoa为_itoa
            strcpy(sfpaddr, "file");
            strcat(sfpaddr, str);
...
int ExistD(char* dirname)  /*Whether DirName Exist,Exist-i,Not Exist-0*/
{
    ...
    int exist = 0;
    for (i = 0; i < ucount; i++)
        if (strcmp(my_strupr(ufd[i]->ufdname), my_strupr(dirname)) == 0)
        {
            exist = 1;
            break;
        }
...
void CdF() /*Exchange Dir*/
{
    ...
    fgets(dname, sizeof(dname), stdin); // 替换gets
    dname[strcspn(dname, "\n")] = '\0';
    ltrim(rtrim(dname));
    if (ExistD(dname) >= 0)  strcpy(dirname, my_strupr(dname));
    else if (strcmp(my_strupr(dname), "CD..") == 0)  strcpy(ltrim(rtrim(dirname)), "");
    else printf("\nError.\'%s\' does not exist.\n", dname);
}

void CreateF()  /*Create File*/
{
    int new_fpaddrno, flag = 1, i; // 重命名fpaddrno为new_fpaddrno（解决变量冲突）
    ...
    if (strcmp(my_strupr(dirname), my_strupr(username)) != 0)
    {
    ...
    else
    {
        ...
        fgets(fname, sizeof(fname), stdin); // 替换gets
        fname[strcspn(fname, "\n")] = '\0';
        ltrim(rtrim(fname));
        if (ExistF(fname) >= 0)
        {
        ...
        else
        {
            ...
            fgets(fmode, sizeof(fmode), stdin); // 替换gets
            fmode[strcspn(fmode, "\n")] = '\0';
            ltrim(rtrim(fmode));
            if ((strcmp(fmode, "0") == 0) || (strcmp(fmode, "1") == 0) || (strcmp(fmode, "2") == 0) || (strcmp(fmode, "3") == 0))
            {
                new_fpaddrno = FindPANo(); // 使用新变量名
                if (new_fpaddrno >= 0)
                {
                    i = ExistD(username);
                    strcpy(ufd[i]->ufdfile[fcount[i]].fname, fname);
                    ufd[i]->ufdfile[fcount[i]].fpaddr = new_fpaddrno; // 使用新变量名
                    ...
                    _itoa(new_fpaddrno, str1, 10); // 替换itoa为_itoa
                    strcat(str, str1);
                    ...
                    while (flag)
                    {
                        ...
                        if (strcmp(my_strupr(a), "Y") == 0)
                        {
                            fp_file = fopen(str, "wb+");
                            ufd[i]->ufdfile[fcount[i] - 1].flength = WriteF1();
                            flag = 0;
                        }
                        else if (strcmp(my_strupr(a), "N") == 0) { flag = 0; wgetchar = 1; }
                    }
...
int ExistF(char* filename)  /*Whether FileName Exist,Exist-i,Not Exist-0*/
{
    ...
    j = ExistD(dirname);
    for (i = 0; i < fcount[j]; i++)
        if (strcmp(my_strupr(ufd[j]->ufdfile[i].fname), my_strupr(filename)) == 0)
        {
            exist = 1;
            break;
        }
            ...
            else
            {
                while (flag)
                {
                    ...
                    if (strcmp(my_strupr(a), "Y") == 0)
                    {
                        ...
                        _itoa(ufd[k]->ufdfile[i].fpaddr, str, 10); // 替换itoa为_itoa
                    ...
                    else if (strcmp(my_strupr(a), "N") == 0)
                    {
...
void ModifyFM() /*Modify FileMode*/
{
    ...
    if (strcmp(my_strupr(dirname), my_strupr(username)) != 0) { printf("\nError.You can only modify filemode in yourself dir.\n"); wgetchar = 1; }
    else
    {
        printf("\nPlease input FileName:");
        fgets(fname, sizeof(fname), stdin); // 替换gets
...
void ReadF() /*Read File*/
{
    ...
    if (strcmp(my_strupr(ltrim(rtrim(dirname))), "") == 0) { printf("\nError.Please convert to ufd dir before read.\n"); wgetchar = 1; return; }
            ...
            if (ifopen[k][i].ifopen == 1)
            {
                if ((ifopen[k][i].openmode == 1) || (ifopen[k][i].openmode == 2))
                {
                    ...
                    if (ufd[k]->ufdfile[i].flength != 0)
                    {
                        ...
                        else if (strcmp(my_strupr(a), "A") == 0)
                        {
                            printf("\nAppend\n");
